    ...
    def __init__(self, model_path, data_struct=None,
        ...
        self.register_buffer('v_template',
                             to_tensor(to_np(data_struct.v_template),
                                       dtype=dtype))
        # move v_template to [0,0]
        # self.t = -1 * to_tensor([0.5*(self.v_template[:,0].max() + self.v_template[:,0].min()), 0.5*(self.v_template[:,1].max() + self.v_template[:,1].min()), 0.5*(self.v_template[:,2].max() + self.v_template[:,2].min())])
        # self.v_template = self.v_template + self.t
