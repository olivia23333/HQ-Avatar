...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim=0, n_layers=5):
        super().__init__()
        
        self.n_layers = n_layers
        h_dim = 256
        # self.shape_code = nn.Parameter(0.002*torch.randn(1, h_dim, 2, 8,8),requires_grad=True)
        self.shape_code = nn.Parameter(0.002*torch.randn(1, h_dim, 8, 8),requires_grad=True)

        # Upsampling 3D
        lin = nn.Linear(z_dim+c_dim, h_dim*2) # activation='relu')
        setattr(self, "lin_code", lin)
        if c_dim != 0:
            self.lin_label = nn.Linear(c_dim, h_dim)
        ...
        for l in range(n_layers):
            if l == n_layers - 1:
                ...
                h_dim_next = max(h_dim//2, 144)
            enc = nn.Conv2d(h_dim, h_dim_next, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, z, c=None):
        ...
        # z = normalize_2nd(z)
        if c != None:
            c = normalize_2nd(self.lin_label(c))
            z = torch.cat([z, c], dim=-1)

        h = self.shape_code.expand(b, -1,-1,-1).clone()
        a = self.lin_code(z)
        h = actvn( adaIN(h,a) )
        ...
        for l in range(self.n_layers):
            ...
            h = F.upsample(h, scale_factor=2, mode='bilinear', align_corners=True)
            h = enc(h)
            ...
            # if l == self.n_layers-3:
            #     low_h = F.upsample(h, scale_factor=4, mode='nearest')
                # low_h = h
        return [low_h, h]
...
def normalize_2nd(x, dim=1, eps=1e-8):
